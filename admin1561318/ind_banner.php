...
?>
	<div class="page-content">
		<div class="content container">
			<div class="row">
			  <form name="form2" method="post" action="ind_banner_sort.php">  
				<div class="col-lg-12">
					<div class="widget">
						<div class="widget-header" style="height: 90px;"> <i class="icon-list-ol"></i>
							<h3>首頁下方輪播banner</h3>
							
<a class="btn btn-danger fancybox" data-fancybox-type="iframe" href="ind_banner_add.php" style="float: right;margin-top: 14px;margin-right: 40px;">新增</a>
							...
							<div class="body">
								<table class="table table-striped">
									...
									<tbody class="tbodyC">
										...
										$phpfile = 'index.php?ct=ind_banner';//使用頁面檔案
										...
										<tr>
											<td style="vertical-align:middle;">
												 <img src="./goods_pic/<?=$rows["banner_pic_s"]; ?>" height="60" border="1" style=" border-color:#666666; border-style:solid;" onerror="this.src='../goods_pic/defpic.jpg'" >
											</td>
											<td style="vertical-align:middle;">
												<span style="font-size:16px;"><?=$rows["banner_title"]; ?></br><?=$rows["banner_link"]; ?></span>
											</td>
											<td style="vertical-align:middle;">
												 <input type="hidden" name="ck_<?=$rows["banner_no"];?>" id="ck_<?=$rows["banner_no"];?>" value="<?=$rows["banner_sort"];?>">
                      <input name="<?=$rows["banner_no"];?>" type="text" id="<?=$rows["banner_no"];?>" style="width:40px;" value="<?=$rows["banner_sort"];?>">
											</td>
											<td align="right" style="vertical-align:middle;text-align: center;">
												<a class="btn btn-primary btn-xs fancybox" data-fancybox-type="iframe" href="ind_banner_edit.php?no=<?=$rows["banner_no"];?>">修改</a>
												<a class="btn btn-danger btn-xs" onclick="if(window.confirm('確定要刪除？一旦刪除則無法復原。')) location.href='ind_banner_del_ok.php?no=<?=$rows["banner_no"];?>&bpic=<?=$rows["banner_pic_b"];?>&spic=<?=$rows["banner_pic_s"];?>';">刪除</a>
											</td>
										</tr>
										<?php	
										}
